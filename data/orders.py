...
def normalize_selected_field_ids(value):
    if value is None:
        return None
    if not isinstance(value, list):
        raise ApiError("invalid_selected_field_ids", status_code=400)
    field_ids = []
    for item in value:
        if not isinstance(item, str) or not item.strip():
            raise ApiError("invalid_selected_field_ids", status_code=400)
        field_ids.append(item.strip())
    if not field_ids:
        raise ApiError("selected_fields_required", status_code=400)
    if len(set(field_ids)) != len(field_ids):
        raise ApiError("duplicate_selected_field_ids", status_code=400)
    if len(field_ids) > MAX_PAID_FIELDS_PER_ORDER:
        raise ApiError("too_many_selected_fields", status_code=400)
    return field_ids


def fields_for_exact_selection(field_ids, profile_ids, wanted_fields):
    fields_by_id = {field["id"]: field for field in repository.get_fields_by_ids(field_ids)}
    quote_profile_ids = set(profile_ids)
    wanted = set(wanted_fields)
    fields = []
    for field_id in field_ids:
        field = fields_by_id.get(field_id)
        if not field:
            raise ApiError("selected_field_not_found", status_code=400)
        if field.get("userId") not in quote_profile_ids:
            raise ApiError("selected_field_outside_quote", status_code=400)
        if field.get("kind") not in wanted:
            raise ApiError("selected_field_not_requested", status_code=400)
        if not is_purchasable(field):
            raise ApiError("selected_field_not_purchasable", status_code=400)
        fields.append(field)
    return fields
...
def create_order(input_data, owner_wallets=None):
    ...
    if selected_field_ids_input and (not stored_quote or stored_quote.get("prompt") != input_data["prompt"]):
        raise ApiError("quote_required_for_selected_fields", status_code=400)
    ...
    wanted_fields = input_data["wantedFields"]
    if selected_field_ids_input:
        fields = fields_for_exact_selection(selected_field_ids_input, profile_ids, wanted_fields)
    else:
        candidates = build_candidates(profile_ids, input_data.get("selectedMatchRefs") or [])
        wanted = set(wanted_fields)
        fields = [
            field
